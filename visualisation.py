...
def run_simulation(A, B, label="Simulation"):
    ...
    print(f"Вклад штрафа за перемещение: {100 * distance_error:.3f}")
...
# Симуляция с подобранными параметрами
print("Запуск симуляции с подобранными параметрами...")
optimal_A, optimal_B = 0.100, 0.100
energy_opt, energy_per_step_opt, dist_opt, height_opt = run_simulation(
    optimal_A, optimal_B, label="Подобранные параметры (A=0.100, B=0.100)"
)
...
plt.plot(time_steps, energy_per_step_opt, label="Подобранные параметры (A=0.100, B=0.100)", color='blue')
